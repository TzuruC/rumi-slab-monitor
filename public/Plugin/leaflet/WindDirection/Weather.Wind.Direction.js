/*
 * Weather Class - Wind Direction
 */
class WindDirection {
	/**
	   * 參數
	   * @param  {L.LatLng} latlng 經緯度
	   * @param  {Number} speed  風速（m/s）
	   * @param  {Number} dir    風向（度）
...
var WindLayer = L.CanvasLayer.extend({
	...
	onDrawLayer: function (info) {
	    ...
	    */ 
	    ...
	    var Limit_N = Math.ceil(map.getBounds()._northEast.lat);
	    var Limit_S = Math.floor(map.getBounds()._southWest.lat);
	    
	    var Limit_E = Math.ceil(map.getBounds()._northEast.lng);
	    var Limit_W = Math.floor(map.getBounds()._southWest.lng); 
	    
	    
	    //	緯度是90度~-90度 每0.5度為一個刻度
	    //	sortData陣列的大小為361
	    //	sortData[0] 是 90度
	    //	sortData[361] 是 -90度
	    
	    Limit_N = (90 - Limit_N) * 2 ;
	    Limit_S = (90 - Limit_S) * 2 ;
	    
	    Limit_E = (180 + Limit_E) * 2 ;
	    Limit_W = (180 + Limit_W) * 2 ;
	    
	    // 按纬度绘制
	    var latPts, latlng, lLatLng, rLatLng, speed, dir, windobj, lwindobj, rwindobj;
	    
	    
	    for(let i = Limit_N; i < Limit_S; i += latOffset) {
	    	latPts = sortData[i];
	    	
	    	//	經度有跨換日線問題!!
	    	if(Limit_E>721 || Limit_W < 0){
	    		
	    		//	有可能有2種情況
	    		//	1.	只發生 Limit_E>721 或 Limit_W < 0 其中一種
	    		//	2.	Limit_E>721 和 Limit_W < 0 同時發生
	    		...
	    		if(Limit_E>721 && Limit_W < 0){
	    			...
	    			for(let j = 0, lenj = latPts.length; j < lenj; j += lngOffset) {
	    	    		...
	    	    		windobj = new WindDirection(latlng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
	    	    		...
	    	    		if(windobj.options.isDrawLeftRight) {
	    	    			...
	    	    			lwindobj = new WindDirection(lLatLng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
	    	    			rwindobj = new WindDirection(rLatLng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
	    			...
	    			if(Limit_E>721){
	    				for(let j = Limit_W; j < 721; j += lngOffset) {
	    					latlng = L.latLng(latPts[j][this.cfg.lat], latPts[j][this.cfg.lng]);
	    		    		speed = Number(latPts[j][this.cfg.value]);
	    		    		dir = Number(latPts[j][this.cfg.dir]);
	    		    		windobj = new WindDirection(latlng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
	    				...
	    				if(windobj.options.isDrawLeftRight) {
	    					for(let j=0;j<Limit_E-721;j += lngOffset){
	    				...
	    				for(let j = 0; j < Limit_E; j += lngOffset) {
	    		    		latlng = L.latLng(latPts[j][this.cfg.lat], latPts[j][this.cfg.lng]);
	    		    		speed = Number(latPts[j][this.cfg.value]);
	    		    		dir = Number(latPts[j][this.cfg.dir]);
	    		    		windobj = new WindDirection(latlng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
	    				...
	    				if(windobj.options.isDrawLeftRight) {
	    					for(let j=Limit_W+721;j<721;j += lngOffset){
	    		...
	    		for(let j = Limit_W; j < Limit_E; j += lngOffset) {
		    		latlng = L.latLng(latPts[j][this.cfg.lat], latPts[j][this.cfg.lng]);
		    		speed = Number(latPts[j][this.cfg.value]);
		    		dir = Number(latPts[j][this.cfg.dir]);
		    		windobj = new WindDirection(latlng, speed, dir, { isDrawLeftRight: this.cfg.isDrawLeftRight });
