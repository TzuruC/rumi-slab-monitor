...
function makeMobileFriendly() {
    $('.sortable')
        ...
        .on('touchend', function (e) {
            ...
            promptForMovement(div);
            e.stopPropagation(); // prevent triggering on parents/ancestors too
        })
...
function promptForMovement(element) {
    const height = 60; // element.height();
    const width = 60; // element.width();
...
// There's a newer version, but it seems to have a broken toHierarchy: https://cdn.jsdelivr.net/gh/ilikenwf/nestedSortable@2.0.0/jquery.mjs.nestedSortable.js

/*
   * jQuery UI Nested Sortable
   * This looks like an edited version of 1.3.5:
   https://codepen.io/kykyy/pen/VNaRKg
   https://codepen.io/hchiam/pen/KKybqEG?editors=0010
   * v 1.3.5 / 21 jun 2012
        ...
        $.extend({}, $.ui.sortable.prototype, {
            ...
            _create: function () {
                ...
                if (!this.element.is(this.options.listType))
                    throw new Error(
                        'nestedSortable: Please check that the listType option is set to your actual list type'
                    );
                ...
                if (this.options.isTree) {
                    ...
                    $(this.items).each(function () {
                        ...
                        if ($li.children(self.options.listType).length) {
                            ...
                            if (
                                !$li.hasClass(self.options.collapsedClass) &&
                                !$li.hasClass(self.options.expandedClass)
                            ) {
            ...
            _mouseDrag: function (event) {
                ...
                if (
                    ...
                    if (
                        o.isTree &&
                        parentItem.children(o.listItem).children('li:visible:not(.ui-sortable-helper)').length < 1
                    ) {
                        parentItem
            ...
            _mouseStop: function (event, noPropagation) {
                ...
                if (!(pid == this._uiHash().item.parent().parent().attr('id') && sort == this._uiHash().item.index())) {
            ...
            toHierarchy: function (options) {
                ...
                $(this.element)
                    .children(o.items)
                    .each(function () {
                        var level = _recursiveItems(this, $(this).attr('data-in'));
                        ret.push(level);
                    });

                return ret;

                function _recursiveItems(item, n) {
                    var id = ($(item).attr(o.attribute || 'id') || '').match(o.expression || /(.+)[-=_](.+)/);
                    var id = $(item).attr('data-id');
                    if (id) {
                        if ($(item).attr('data-in') == 1) {
                            n = 1;
                        } else {
                            n++;
                        }
                        ...
                        if ($(item).children(o.listType).children(o.items).length > 0) {
                            ...
                            $(item)
                                ...
                                .children(o.items)
                                .each(function () {
                                    var level = _recursiveItems(this, n);
